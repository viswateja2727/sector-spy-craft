...
import { ChevronLeft, ChevronRight, Download, Copy, Check, FileDown, Sparkles } from 'lucide-react';
...
export function SlidePreview({ teaser, futurePlans, companyData }: SlidePreviewProps) {
  ...
  const handleDownloadPPT = useCallback(async () => {
    setGenerating(true);
    try {
      // First enhance content with AI if company data is available
      ...
      if (!aiContent && companyData) {
        ...
        aiContent = await enhanceTeaserContent(companyData);
        if (aiContent) {
          setEnhancedContent(aiContent);
        }
      }
      ...
      toast.success('PowerPoint generated successfully!', {
        description: 'Your editable PPT file has been downloaded.',
      });
    } catch (error) {
  ...
  }, [teaser, futurePlans, companyData, enhancedContent]);
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button 
            ...
            disabled={generating}
            className="kelp-gradient text-white"
          >
            {generating ? (
              <>
        ...
        >
          <div className="w-full flex-shrink-0 px-4">
            <BusinessProfileSlide 
              slide={slides[0]} 
              sector={teaser.metadata.sector}
            />
          </div>
          <div className="w-full flex-shrink-0 px-4">
            <FinancialScaleSlide 
              slide={slides[1]} 
              revenueData={teaser.charts.revenueData}
              metrics={metrics}
            />
          </div>
          <div className="w-full flex-shrink-0 px-4">
            <InvestmentHighlightsSlide 
              slide={slides[2]}
